...
Adapts a class constructor into a factory, so classes register without a `(deps) => new Foo(...)` wrapper. It takes a single deps object and stays fully type-safe.
...
Adapts a class with a positional constructor: deps are spread into the constructor in `deps` order. Only the instance type is inferred, so `deps` must list the constructor's parameters in order.
